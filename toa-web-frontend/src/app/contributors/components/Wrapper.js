...
import { ContributorsData, PeronsData, TeamsData } from "@/app/utils/swr/fetchdata";
...
const Wrapper = ({ teamPanelData, currentLanguage }) => {
  ...
  const {dataSwr, isLoading} = ContributorsData({currentLanguage: currentLanguage})
  ...
  return (
    <div className="w-screen h-full">
      ...
      <DContainer>
        {(!isLoading && !isLoadingP && Boolean(teamsData)) && <div className="w-full h-full flex flex-col items-center gap-4 pt-8">
          {teamsData && teamsData.map((val, idx) => {
            return (
